...
export default function HelpView() {
  return (
    <div className="flex flex-col h-full overflow-y-auto px-6 py-4 gap-4">
      <h2 className="text-lg font-semibold text-text dark:text-text-dark">Help & Instructions</h2>
      <div className="flex flex-col gap-3">
        {sections.map((s) => (
          <MacCard key={s.title}>
            <div className="p-4">
              <h3 className="text-sm font-semibold text-text dark:text-text-dark mb-1.5">{s.title}</h3>
              <p className="text-sm text-text-3 dark:text-text-3-dark leading-relaxed">{s.content}</p>
            </div>
          </MacCard>
        ))}
      </div>
